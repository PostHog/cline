...
export class ShellDetector {
    ...
    // -----------------------------------------------------
    private getWindowsTerminalConfig() {
        try {
    ...
    private getLinuxTerminalConfig() {
        try {
            const config = vscode.workspace.getConfiguration('terminal.integrated')
    ...
    private getWindowsShellFromVSCode(): string | null {
        const { defaultProfileName, profiles } = this.getWindowsTerminalConfig()
    ...
    private getMacShellFromVSCode(): string | null {
        const { defaultProfileName, profiles } = this.getMacTerminalConfig()
        if (!defaultProfileName) {
            return null
        }
    ...
    private getLinuxShellFromVSCode(): string | null {
        const { defaultProfileName, profiles } = this.getLinuxTerminalConfig()
        if (!defaultProfileName) {
            return null
        }
     ...
     */
    private getShellFromUserInfo(): string | null {
        try {
            const { shell } = this.getUserInfo()
            return shell || null
        } catch {
            return null
        }
    ...
    private getShellFromEnv(): string | null {
        const { env } = process
    ...
    getShell(): string {
        ...
        if (process.platform === 'win32') {
            ...
            const windowsShell = this.getWindowsShellFromVSCode()
            if (windowsShell) {
                return windowsShell
            }
        } else if (process.platform === 'darwin') {
            // macOS from VS Code
            const macShell = this.getMacShellFromVSCode()
            if (macShell) {
                return macShell
            }
        } else if (process.platform === 'linux') {
            // Linux from VS Code
            const linuxShell = this.getLinuxShellFromVSCode()
            if (linuxShell) {
                return linuxShell
            }
        }
        ...
        const userInfoShell = this.getShellFromUserInfo()
        if (userInfoShell) {
            return userInfoShell
        }

        // 3. If still nothing, try environment variable
        const envShell = this.getShellFromEnv()
        if (envShell) {
            return envShell
        }
